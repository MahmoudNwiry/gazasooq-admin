...
const ExportUsers: React.FC<ExportUsersProps> = ({ users, isOpen, onClose }) => {
  ...
  const handleExport = async () => {
    ...
    const exportData = users.map(user => {
      ...
      if (exportFields.basicInfo) {
        ...
        data.الدور = user.role;
      }
      
      if (exportFields.contactInfo) {
        data['رقم الهاتف'] = user.phone;
        data.الموقع = user.location;
      }
      
      if (exportFields.status) {
        data.الحالة = user.status === 'active' ? 'نشط' : 
                     user.status === 'inactive' ? 'غير نشط' : 
                     user.status === 'suspended' ? 'محظور' : user.status;
      }
      
      if (exportFields.dates) {
        data['تاريخ التسجيل'] = user.joinDate;
        data['آخر دخول'] = user.lastLogin;
      }
      
      if (exportFields.statistics) {
        data['عدد الطلبات'] = user.totalOrders || 0;
        data['إجمالي المشتريات'] = user.totalSpent || '0 ش.ج';
      }
  ...
  const generateExcelFile = (data: any[]) => {
    ...
    downloadFile('users_export.xlsx');
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] overflow-hidden border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {users.length} مستخدم جاهز للتصدير
              </p>
            </div>
          </div>
          <button
            ...
            <div className="space-y-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
              <label className="flex items-center gap-3 cursor-pointer hover:bg-white dark:hover:bg-gray-600/50 p-2 rounded-lg transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">الاسم والدور</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer hover:bg-white dark:hover:bg-gray-600/50 p-2 rounded-lg transition-colors">
                <input
                  ...
                  onChange={(e) => setExportFields(prev => ({ ...prev, contactInfo: e.target.checked }))}
                  className="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                />
                <div className="flex-1">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">الهاتف والموقع</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer hover:bg-white dark:hover:bg-gray-600/50 p-2 rounded-lg transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">نشط، غير نشط، محظور</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer hover:bg-white dark:hover:bg-gray-600/50 p-2 rounded-lg transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">التسجيل وآخر دخول</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer hover:bg-white dark:hover:bg-gray-600/50 p-2 rounded-lg transition-colors">
                <input
                  type="checkbox"
                  checked={exportFields.statistics}
                  onChange={(e) => setExportFields(prev => ({ ...prev, statistics: e.target.checked }))}
                  className="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                />
                <div className="flex-1">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">عدد الطلبات والمشتريات</p>
                </div>
              </label>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            <span className="font-medium">{users.length}</span> مستخدم • تنسيق 
