...
import { SimpleLoader, LoadingScreen } from "../../components/common";
...
export default function EditPlan() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        setDuration(data.duration || 'شهري');
        setFeatures(Array.isArray(data.features) ? data.features.join(', ') : '');
        setAllowedProducts(data.limits?.products || 0);
        setAllowedOffers(data.limits?.offers || 0);
      } catch (error: any) {
        console.error(error);
        if (error.response?.status === 404) {
          toast.error("الخطة غير موجودة");
        } else {
          toast.error("حدث خطأ أثناء جلب بيانات الخطة");
        }
  ...
  const sendData = async () => {
    ...
    try {
      ...
      const validationResult = SubscriptionPlanUpdateSchema.safeParse({...body, features});
      ...
      const response = await axiosInstance.put(`/owner/subscription-plan/${spid}`, body);
  ...
  if (initialLoading) {
    return <LoadingScreen title="تحميل بيانات الخطة" subtitle="جارٍ تحميل بيانات الخطة للتعديل..." />;
  }

  return (
    <div>
      <PageMeta
        title="تعديل خطة الاشتراك - سوق إكسبرس"
        description="تعديل بيانات خطة الاشتراك"
      />
      <PageBreadcrumb pageTitle="تعديل خطة الاشتراك" />
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">قم بتعديل بيانات الخطة حسب الحاجة</p>
          </div>
          <button
            onClick={() => navigate(-1)}
